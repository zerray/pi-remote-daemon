...
export default function remoteDaemonExtension(pi: ExtensionAPI): void {
  pi.registerCommand("remote-daemon", {
    ...
    handler: async (args, ctx) => {
      ...
      const result = await pi.exec("pi-remote-daemon", [subcommand]);
      const output = result.stdout.trim() || result.stderr.trim() || `pi-remote-daemon ${subcommand} exited ${result.code}`;
