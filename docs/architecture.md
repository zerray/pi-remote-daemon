...
- Receive daemon-forwarded prompt, abort, compact, tree-refresh, tree-navigation, fork, and clone commands and apply them to the current live TUI runtime through Pi extension APIs.
- Report asynchronous compact, tree-navigation, fork, clone, and session-replacement results back to the daemon for iOS WebSocket subscribers.
...
- Relay prompt, abort, compact, tree-refresh, tree-navigation, fork, and clone requests from iOS to the TUI extension that owns the target session.
- Forward TUI-reported compact, tree, navigation, fork, clone, and replacement results to subscribed iOS clients.
- Store the latest TUI-reported runtime-status snapshot and tree snapshot for each active session and include the runtime status in session state sent to iOS.
...
Multiple TUI processes may enable remote control at the same time. Each active session has one owning TUI control channel. The daemon rejects prompt, abort, tree, fork, clone, and compact requests for sessions without an active owner.
